...
public class SecurityConfiguration {
	...
	// deprecated --> 더 이상 지원하지 않을 문법들. 빗금..
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
		...
		// 객체::메소드명 (lambda 식에서 제공해주는 method reference 연산자)
		...
		// 4. 사용자 지정 페이지로 인증 받을 수 있도록 메소드 설정
			.formLogin((login) -> 
					login.loginPage("/")
					.loginProcessingUrl("/login-process") // = @PostMapping
					.usernameParameter("userid")
					.passwordParameter("pw")
					// build된 user == userid, pw인지 확인
					.defaultSuccessUrl("/dashboard") // = redirect:/dashboard
					.permitAll());
	...
	// --> DB에 있는 데이터를 가져오려고 했더니, 암호화하는 Encoder없다 ~~ error 발생
	@Bean
